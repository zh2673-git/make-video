...
> 日期：2026-09-24 ~ 09-25 | 验证构成：管线回归验证（验证用工程已清理）+ 示例工程端到端（`projects/makevideo-使用指南`；v0.0.7 起由双模式示范工程 `makevideo-创意一分钟` / `makevideo-规则一分钟` 取代，旧工程已删除，历史记录保留）
...
## 十一、v0.0.6 增量验证（2026-09-25）

新能力：**双模式架构（画面实现轴）**——`mode=rule`（13 型预制件，缺省）/ `mode=creative`（每镜 `scenes/<名>.tsx` LLM 场景代码），片级二选一；错误码新增 **E8 创意场景校验失败**（装载缺失 / import 白名单 / tsc 编译 / 试帧四类）。

### P13 前置（创意场景三道闸）

| # | 检查项 | 结果 |
|---|---|---|
| 1 | 装载：6 个场景文件齐全性检查，缺失即 E8 附缺失清单 | ✅ |
| 2 | import 白名单：仅 react/remotion/@remotion/* + 相对导入一层 | ✅ |
| 3 | tsc 编译：engine strict 全量（含生成注册表 custom.gen.ts） | ✅ |
| 4 | 逐镜试帧：6 镜中点帧 remotion still 全过；内容指纹+帧表缓存命中秒过 | ✅ |

### Q13 双模式对比实验（同一旁白逐字相同，唯一变量 = 画面实现）

| 工程 | 模式 | 分镜 | 时长 | 特征 |
|---|---|---|---|---|
| makevideo-创意一分钟 | creative | 6 镜 / 6 个定制场景 | 53.7s | cover 弹入+光带生长 / pipeline 连线逐段点亮 / numbers 非对称数字面+水印 / terminal 打字机+光标呼吸 / swatches 色板矩阵+扫光 / endcard 公式页+星点 |
| makevideo-规则一分钟 | rule | 6 镜 / 6 型预制件 | 53.7s | title / flow / stat / code / quote / ending |

- 旁白逐字相同 → 时长完全一致（53.7s），验证**时间轴层两模式共享**、画面实现解耦。
- 创意片 6 个场景全部为内容定制（讲管线画流动、讲命令敲终端、讲数字做数字面），无固定版式套路；规则片为预制件直接消费。
- 主题兼容：创意工程 `--theme warmedu` 换主题零代码改动跑通（场景颜色/字号全走 CSS 变量）。

### I 增量（v0.0.6 追加）

| 不变量 | 结果 | 说明 |
|---|---|---|
| 规则模式向后兼容 | ✅ | 存量工程 validate 零改动通过；mode 缺省 rule |
| 创意片拦截规则版式 | ✅ | type=bullets 进 creative → E2「创意模式不允许规则版式」（防偷懒铁闸） |
| 规则片拦截创意场景名 | ✅ | type=cover 进 rule → E2 附「mode: creative」提示 |
| 白名单拦截 fs/lodash/越界 | ✅ | node:fs、任意 npm 包、`../../` 相对越界均 E8 附文件行号 |
| 试帧缓存幂等 | ✅ | 场景代码未变更时缓存命中跳过（键=内容指纹+帧表） |
| 生成注册表防串场 | ✅ | 装载前清空 engine/src/custom/ 残留，custom.gen.ts 全量重生成 |

### v0.0.6 已知限制

1. 创意场景质量依赖 LLM 构图能力，三道闸保证「不崩、不越权、可复现」，不评价美学；试帧 PNG 可供人审后回改。
2. 逐镜试帧仅在场景代码或帧表变更时重试；主题 token 变更不触发（token 只影响取值不影响可渲染性）。

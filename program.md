...
> 2026-09-25 增量：v0.0.6 **双模式架构**（画面实现轴）——**规则模式**（13 型预制件，稳定快出、可接受单调）与**创意模式**（每镜画面由 LLM 按内容理解写 Remotion 场景代码，去模板化）**片级二选一**（project.json `mode`）。明确不做逐镜混用：混排会让 LLM 偷懒回退规则版式，创意名存实亡。创意模式配校验闭环（tsc + import 白名单 + 逐镜试帧，E8）兜底，时间轴层（TTS/对齐/字幕/渲染）完全复用。研究方向 6 落地。
...
- **双模式画面实现**（v0.0.6）：`mode=rule`（缺省）走 13 型预制件——稳定快出，接受单调；`mode=creative` 每镜画面 = 工程 `scenes/<场景名>.tsx` 场景代码——LLM 按内容理解定制 SVG/构图/动效，无固定版式套路。片级二选一，不做逐镜混用（防 LLM 偷懒回退）。时间轴层两模式完全共享。
...
6. ~~画面层去模板化~~ **已落地（v0.0.6）**：双模式架构。规则模式 = 13 型预制件（兼容层/快捷档）；创意模式 = 逐镜 `scenes/<名>.tsx` 场景代码（LLM 按内容理解定制，讲稿 SCENE 头 type 位即场景名），装载为 `engine/src/custom/` + 生成的静态注册表（打包期可见），校验闭环（tsc 编译 + import 白名单 + 逐镜试帧 → E8 结构化错误，LLM 外环消费回改）保证创意代码与规则代码走同一条确定性管线。主题 token（79 套）与薄原语画笔 SDK（primitives）为创意层共用画笔；compose 三模式（v0.0.5）服务于规则模式的分镜决策，与创意模式分属两条创作路径。

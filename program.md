...
> 2026-09-25 增量：v0.0.5 决策层开放——**SKILL.md**（LLM 创作引导，skill 化第一步）与 **compose --ai**（LLM 排版决策后端，OpenAI 兼容协议 + `--prompt-out` 离线任务包；生成即 E2 校验，规则链保留为离线兜底），研究方向 5 落地。
...
- **纯文本进、分镜出**：`compose` 自动分镜引擎（v0.0.2）——纯文本讲稿按启发式规则链（code → table → chart → timeline → compare → flow → quote → bullets 兜底）自动选版式生成 DSL 草稿，人微调后进管线。
- **风格进、主题出**：引入 design skill（canvas-design 设计哲学方法论），把"风格"沉淀为可复用主题库，换主题即换风格，内容零改动。
- **动效不重样**：主题可选 variants 变体池（v0.0.2），同版式多套入场动效按出现序号轮换（相邻不重复），消除成片机械感；缺省段向后兼容。
...
5. ~~自动分镜启发式的泛化~~ **已落地（v0.0.5）**：`compose --ai` 新增 LLM 排版决策后端——输入仍是人的原文（逐字保留，LLM 只有排版权），输出 DSL 草稿经 E2 校验护栏 + 失败回喂重试；SKILL.md 作为 LLM 引导单一源头（人写讲稿与 --ai 共用）；规则链降为 `--offline` 等效缺省档。

## 项目愿景

- 短期：工具与主题系统落地，示例工程端到端出片验证。
- 中期：沉淀为 skill（结合 essence-workshop 工作流），"讲稿+主题"两参数出片。**第一步已落（v0.0.5）**：根目录 `SKILL.md` = 任何 LLM 读后即可正确产出讲稿的创作引导（DSL 速查/版式决策表/图标清单/质量纪律）。

> 源头：批量移植（[awesome-design-md](https://github.com/VoltAgent/awesome-design-md) · design-md/sentry）| `import-themes` 自动转换（fm 解析）
> 转换规则：过滤交互态与响应式断点（固定 1920×1080 画布）；字号阶映射视频画布常量；动效为默认档（见文末第 10 节）。
> 推导备注：primaryDark=darken(0.28); primaryLight=lighten(0.18); surface=按 ink 亮度推导; surfaceCard=mix(0.06); surfaceAlt=mix(0.12); inkMuted=mix(0.42); 对比度弱：primary 与 ink 亮度接近
> 完整范式见 govgold 版。
